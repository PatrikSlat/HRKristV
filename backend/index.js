...
async function verifyRecaptcha(req, res, next) {
  ...
  try {
    ...
    // Ako koristiš reCAPTCHA v2
    if (!success) {
      return res.status(403).json({ message: "reCAPTCHA verifikacija nije uspjela. Pokušajte ponovo." });
    }
    next();
  } catch (error) {
...
// RUTA ZA REGISTRACIJU ORGANIZATORA + reCAPTCHA MIDDLEWARE
app.post("/api/registracija-zahtjev/organizator", verifyRecaptcha, async (req, res) => {
  ...
  //MAIL PRIKAZ...
  const emailHtml = `<h1>Novi zahtjev za registraciju organizatora</h1> <ul> <li><strong>Naziv udruge/organizacije:</strong> ${nazivUdruge}</li> <li><strong>OIB udruge:</strong> ${oibUdruge}</li> <li><strong>Kontakt osoba:</strong> ${kontaktOsoba}</li> <li><strong>Email:</strong> ${email}</li> ${telefon ? `<li><strong>Telefon:</strong> ${telefon}</li>` : ''} ${poruka ? `<li><strong>Poruka:</strong> ${poruka}</li>` : ''} </ul> <p>Molimo Admina da odradi svoj posao !</p>`;
  const mailOptions = {
